...
Original file is located at
    https://colab.research.google.com/drive/1XCKdHcZJbxXWq6g1fQX3Pg63S6ZnfhOF
"""
...
model = ElectraModel.from_pretrained("google/electra-base-discriminator",output_hidden_states = True)
tokenizer = ElectraTokenizer.from_pretrained("google/electra-base-discriminator",use_fast=True)
for word in words:
  tokenizer.add_tokens(word)

model.resize_token_embeddings(len(tokenizer))

def electra_text_preparation(text, tokenizer):
...
def get_electra_embeddings(tokens_tensor, segments_tensors, model):
    with torch.no_grad():
...
for text,word,ids in zip(texts1,words,id):
    tokenizer.add_tokens(word)
    tokenized_text, tokens_tensor, segments_tensors = electra_text_preparation(text, tokenizer)
    list_token_embeddings = get_electra_embeddings(tokens_tensor, segments_tensors, model)
...
with open('/content/drive/MyDrive/TempoWiC_Starting_Kit Evaluation/TempoWiC_Starting_Kit/predictions/test.electra-preds.tsv', 'w') as preds_f:
